...
public class UserService {
    ...
    public Map<String, String> updateProfile(User user, String oldPassword, String newPassword, String confirmPassword, User sessionUser) {
        Map<String, String> errors = new HashMap<>();

        // Fetch current user details from DB to compare username and email
        User dbUser = userRepository.findById(sessionUser.getId()).orElse(null);
        String currentName = (dbUser != null) ? dbUser.getName() : "";
        String currentEmail = (dbUser != null) ? dbUser.getEmail() : "";
        ...
        if (name == null || name.trim().isEmpty()) {
            ...
            errors.put("name", "El nom d'usuari ha de tenir entre 5 i 15 caràcters.");
        } else if (!name.equals(currentName) && userRepository.existsByUsername(name)) {
            errors.put("name", "El nom d'usuari ja existeix.");
        }
        ...
        if (email == null || email.trim().isEmpty()) {
            ...
            errors.put("email", "Format de correu electrònic invàlid.");
        } else if (!email.equals(currentEmail) && userRepository.existsByEmail(email)) {
            errors.put("email", "El correu electrònic ja està registrat.");
        }

        boolean changingPassword = (oldPassword != null && !oldPassword.isEmpty())
                || (newPassword != null && !newPassword.isEmpty())
                || (confirmPassword != null && !confirmPassword.isEmpty());

        if (changingPassword) {
            User dbUserWithPass = userRepository.findByName(currentName).orElse(null);
            if (dbUserWithPass == null || !dbUserWithPass.getPassword().equals(oldPassword)) {
                errors.put("oldPassword", "La contrasenya antiga no és correcta.");
            }
            if (newPassword == null || !newPassword.matches(PASSWORD_REGEX)) {
                errors.put("password", "La nova contrasenya ha de tenir almenys 8 caràcters, incloent majúscules, minúscules, números i caràcters especials.");
            }
            if (newPassword != null && !newPassword.equals(confirmPassword)) {
                errors.put("confirmPassword", "Les contrasenyes no coincideixen.");
            }
            if (errors.isEmpty()) {
            ...
            User dbUserWithPass = userRepository.findByName(currentName).orElse(null);
            if (dbUserWithPass != null) {
                user.setPassword(dbUserWithPass.getPassword());
            } else {
